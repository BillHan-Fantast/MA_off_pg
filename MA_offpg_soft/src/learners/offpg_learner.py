...
class OffPGLearner:
    ...
    def train_critic(self, on_batch, best_batch=None, log=None):
        ...
        for i in range(max_t):
            agent_outs, _ = self.mac.forward(on_batch, t=i)
            mac_out.append(agent_outs)
        ...
        mac_out = mac_out / mac_out.sum(dim=-1, keepdim=True)
        mac_out[avail_actions == 0] = 0
        log_mac = th.log(th.gather(mac_out, 3, actions).squeeze(3) + 1e-8).sum(dim=2, keepdim=True)
        ...
        target_q = build_td_lambda_targets(rewards, terminated, log_mac, mask, targets_taken, self.args.beta, self.args.gamma, self.args.td_lambda).repeat(1, 1, self.n_agents)
    ...
    def train_critic_best(self, batch):
        ...
        mac_out = mac_out / mac_out.sum(dim=-1, keepdim=True)
        mac_out[avail_actions == 0] = 0
        critic_mac = th.gather(mac_out, 3, actions).squeeze(3)
        log_mac = th.log(critic_mac + 1e-8).sum(dim=2, keepdim=True)[:, :-1]
        critic_mac = critic_mac.prod(dim=2, keepdim=True)
        ...
        td_q = (rewards - self.args.beta * log_mac +  self.args.gamma * exp_q[:, 1:] - targets_taken[:, :-1]) * mask
