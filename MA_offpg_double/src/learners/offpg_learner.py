...
class OffPGLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.double_critic_optimiser = RMSprop(params=self.double_critic_params, lr=args.critic_lr, alpha=args.optim_alpha, eps=args.optim_eps)
    ...
    def train_critic(self, on_batch, best_batch=None, log=None):
        ...
        double_target_q_vals = self.double_target_critic.forward(target_inputs).detach()
        targets_taken = th.mean(th.gather(target_q_vals, dim=3, index=actions).squeeze(3), dim=2, keepdim=True)
        double_targets_taken = th.mean(th.gather(double_target_q_vals, dim=3, index=actions).squeeze(3), dim=2, keepdim=True)
        ...
        double_target_q = build_td_lambda_targets(rewards, terminated, mask, double_targets_taken, self.n_agents, self.args.gamma, self.args.td_lambda).repeat(1, 1, self.n_agents)
        
        inputs = self.critic._build_inputs(on_batch, bs, max_t)

        if best_batch is not None:
            best_target_q, best_double_target_q, best_inputs, best_mask, best_actions= self.train_critic_best(best_batch)
            target_q = th.cat((target_q, best_target_q), dim=0)
            double_target_q = th.cat((double_target_q, best_double_target_q), dim=0)
        ...
        for t in range(max_t - 1):
            ...
            double_q_vals = th.gather(double_q_vals, 3, index=actions[:, t:t+1]).squeeze(3)
            ...
            grad_norm = th.nn.utils.clip_grad_norm_(self.double_critic_params, self.args.grad_norm_clip)
            self.double_critic_optimiser.step()
            
            log["critic_loss"].append(double_critic_loss.item())
            log["critic_grad_norm"].append(grad_norm)
            mask_elems = mask_t.sum().item()
            log["td_error_abs"].append((double_q_err.abs().sum().item() / mask_elems))
            log["target_mean"].append((double_target_q_t * mask_t).sum().item() / mask_elems)
            log["q_taken_mean"].append((double_q_vals * mask_t).sum().item() / mask_elems)
    ...
    def train_critic_best(self, batch):
        ...
        double_targets_taken[:, -1] = double_targets_taken[:, -1] * (1 - th.sum(terminated, dim=1))
        double_exp_q[:, -1] = double_exp_q[:, -1] * (1 - th.sum(terminated, dim=1))
        double_targets_taken[:, :-1] = double_targets_taken[:, :-1] * mask
        double_exp_q[:, :-1] = double_exp_q[:, :-1] * mask
        double_td_q = (rewards + self.args.gamma * double_exp_q[:, 1:] - double_targets_taken[:, :-1]) * mask
        ...
        double_target_q =  build_target_q(double_td_q, double_targets_taken[:, :-1], critic_mac, mask, self.args.gamma, self.args.td_lambda, self.args.step).detach().repeat(1, 1, self.n_agents)
    ...
    def build_exp_q(self, batch, mac_out, bs, max_t):
        ...
        double_target_exp_q_vals = self.double_target_critic.forward(inputs).detach()
        double_target_exp_q_vals = th.gather(double_target_exp_q_vals, 4, actions).squeeze(-1).mean(dim=3)
        ...
        double_target_exp_q_vals = th.sum(double_target_exp_q_vals * action_mac, dim=2, keepdim=True) / (th.sum(action_mac, dim=2, keepdim=True) + 1e-10)
    ...
    def save_models(self, path):
        ...
        th.save(self.double_critic.state_dict(), "{}/double_critic.th".format(path))
        th.save(self.agent_optimiser.state_dict(), "{}/agent_opt.th".format(path))
        th.save(self.critic_optimiser.state_dict(), "{}/critic_opt.th".format(path))
        th.save(self.double_critic_optimiser.state_dict(), "{}/double_critic_opt.th".format(path))

    def load_models(self, path):
        self.mac.load_models(path)
        self.critic.load_state_dict(th.load("{}/critic.th".format(path), map_location=lambda storage, loc: storage))
        self.double.load_state_dict(th.load("{}/double_critic.th".format(path), map_location=lambda storage, loc: storage))
        # Not quite right but I don't want to save target networks
        self.target_critic.load_state_dict(self.critic.state_dict())
        self.double_target_critic.load_state_dict(self.double_critic.state_dict())
        self.agent_optimiser.load_state_dict(th.load("{}/agent_opt.th".format(path), map_location=lambda storage, loc: storage))
        self.critic_optimiser.load_state_dict(th.load("{}/critic_opt.th".format(path), map_location=lambda storage, loc: storage))
        self.double_critic_optimiser.load_state_dict(
            th.load("{}/double_critic_opt.th".format(path), map_location=lambda storage, loc: storage))
